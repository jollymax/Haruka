...
const starts = async (haruka = new WAConnection()) => {
    ...
    haruka.browserDescription = [ 'RAKUN_BOTZ.ID', 'Chrome', '3.0' ]
	CFonts.say('Haruka', {
		font: 'block',
    	color: ['#ff9c00'],
    	align: 'center',
	...
	haruka.on('group-participants-update', async (anu) => {
		...
		try {
						...
						if (anu.action == 'add' && !num.includes(haruka.user.jid)) {
							...
							try{
							await sendButLoc(mdata.id, `Welcome @${num.split('@')[0]} to ${mdata.subject}` + '\n' + lang.welcome(), `Welcome Message By ${ownername}`,welcome, [{"buttonId": `.owner`,"buttonText": {"displayText": "WELCOME BOS KU 🤗"},"type": "RESPONSE"}], {contextInfo: { mentionedJid: [num]}})
							} catch {
								await sendButLoc(mdata.id, `Welcome @${num.split('@')[0]} to ${mdata.subject}` + '\n' + lang.welcome(), `Welcome Message By ${ownername}`,ppmem2, [{"buttonId": `.owner`,"buttonText": {"displayText": "WELCOME BOS KU 🤗"},"type": "RESPONSE"}], {contextInfo: { mentionedJid: [num]}})
							}
							...
							try{
							await sendButLoc(mdata.id, `Goodbye @${num.split('@')[0]}\n⌯ָ   ֙Leave from group:\n${mdata.subject}` + '\n' + lang.leave(), `SELAMAT TINGGAL BEBAN GERUP By ${ownername}`,goodbye, [{"buttonId": `.owner`,"buttonText": {"displayText": "SELAMAT TINGGAL BEBAN 🐒"},"type": "RESPONSE"}], {contextInfo: { mentionedJid: [num]}})
							} catch {
								await sendButLoc(mdata.id, `Goodbye @${num.split('@')[0]}\n⌯ָ   ֙Leave from group:\n${mdata.subject}` + '\n' + lang.leave(), `SELAMAT TINGGAL BEBAN GERUP By ${ownername}`,ppmem2, [{"buttonId": `.owner`,"buttonText": {"displayText": "SELAMAT TINGGAL BEBAN 🐒"},"type": "RESPONSE"}], {contextInfo: { mentionedJid: [num]}})
							}
